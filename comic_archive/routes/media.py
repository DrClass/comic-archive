...
def register_media_routes(app: FastAPI, database: Path, library: Path) -> None:
    ...
    def thumbnail(media_id: str):
        ...
        return FileResponse(path, media_type=THUMBNAIL_MIME, headers={"Cache-Control": "private, max-age=604800"})
    ...
    def media(media_id: str):
        ...
        if record is None:
            raise HTTPException(status_code=404, detail="Media not found")
        stored_path, mime_type = record
        return FileResponse(_safe_library_file(library, stored_path), media_type=mime_type)
